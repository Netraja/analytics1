...
str(omni)
nrow(omni)
dim(omni)#dimension tells you rows and columns
...
#Assumptions
par(mfrow=c(2,2))#parameters to draw plots, mf is for multiple frames
plot(fit2)
par(mfrow=c(1,1))

plot(fit2,which=1)  # no pattern, equal variance, drawing only first plot and that is equal to 1
...
omni[c(11,14,15),]#these are the outlyers 

fit3 = lm(sales ~ price + promotion, data=omni[-c(11,14,15),])#removing the extreme points(outlyers)
plot(fit3,4)
summary(fit3)
