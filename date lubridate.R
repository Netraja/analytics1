...
#Date time format(mostly we deal with date format and sometimes date-time format)
memt <- now()
memt
...
dat1 <- ymd("2017-12-23")#to make use of logical function, we are storing it as date rather than as string
dat1
...
now()
now("EST")#Changing the time format
...
yearsdiff <- year(d2) - year(as.Date("2017-02-23"))#reads it as a date, whatever the changes we have made
