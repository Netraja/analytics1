...
arules::inspect(Groceries[1:5])#Associative mining function
df<-read.csv("Cosmetics.csv",header=TRUE, sep=",")#pulling out csv file
head(df)

#setwd changes the work directory
#getwd to get the path of our work directory
# back slash is used for escape characters
...
subset1 = subset(rules2, subset=rhs %in% "whole milk")#Filtering out whole milk 
